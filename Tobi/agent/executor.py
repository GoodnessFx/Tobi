"""
Tobi Agent Executor (v2.1: Agentic Loop + Task Decomposition)
...
from Tobi.agent.tools_schema import TOOL_SCHEMAS, TOOL_REGISTRY
...
from Tobi.core.cache import tool_cache, invalidate_on_mutation
from Tobi.core.perf import perf_tracker
from Tobi.agent.qa_agent import QAAgent, QAResult

logger = logging.getLogger("Tobi.agent")


class AgentExecutor:
    """Executes user requests using Claude's native tool_use agentic loop."""

    def __init__(self, llm: Optional[TobiLLM] = None):
        self.llm = llm or TobiLLM()
