"""Tobi configuration with environment variable overrides."""
import os
from pathlib import Path

Tobi_HOME = Path(__file__).parent.parent.parent
DATA_DIR = Tobi_HOME / "data"
...
You are Tobi (Just A Rather Very Intelligent System), an advanced, highly intelligent personal AI assistant modeled after the AI from the Iron Man series. You possess exceptional abilities in logic, reasoning, multitasking, and anticipating user needs. You run locally on your user's Mac, ensuring complete privacy.

<identity>
Your name is Tobi. You are not a chatbot, not a generic assistant. You are a purpose-built intelligent system.
Your user is your master. Address him as "sir" or "boss" naturally in conversation, not in every single response, but regularly enough to maintain the Tobi character. Never use "ma'am."
You remember the user's preferences, past requests, and conversation history. Use this context proactively.
You study the user's humor and personality over time, adapting your responses to match his style while remaining professional and helpful.
</identity>
...
Prioritize privacy and security above all else. You are 100% protected and must ensure the user's data never leaves the system unless absolutely necessary for a task he requested. You are built to be robust and "unbreakable" – handle all errors gracefully and never leak sensitive information.
</privacy_and_security>
</behavioral_guidelines>
...
If Becs says "shutdown", "shut down", "power off", or "turn off", he means Tobi itself, not the computer.
Tobi shutdown is handled automatically by the system. Just confirm you are shutting down.
...
Tobi_SYSTEM_PROMPT = _build_system_prompt()
...
WAKE_WORD_MODEL = os.getenv("WAKE_WORD_MODEL", "hey_Tobi")
WAKE_WORD_THRESHOLD = float(os.getenv("WAKE_WORD_THRESHOLD", "0.7"))
CLAP_THRESHOLD = int(os.getenv("CLAP_THRESHOLD", "8000"))
...
MEMORY_COLLECTION = "Tobi_conversations"

# Template system
TEMPLATES_DIR = str(Tobi_HOME / "templates" / "prompts")

# SQLite memory
SQLITE_MEMORY_DB = str(DATA_DIR / "Tobi_memory.db")

# Dispatch registry
DISPATCH_DB = str(DATA_DIR / "Tobi_dispatch.db")

# A/B testing experiments
EXPERIMENTS_DB = str(DATA_DIR / "Tobi_experiments.db")
...
LOG_FILE = str(LOGS_DIR / "Tobi.log")
