...
logger = logging.getLogger("Tobi.auth")
...
def initialize_pin() -> str:
    ...
    custom_pin = os.getenv("Tobi_PIN", "").strip()
    if custom_pin:
        if not custom_pin.isdigit() or not (4 <= len(custom_pin) <= 8):
            logger.error(
                "Tobi_PIN must be 4-8 digits. Got: '%s' (%d chars). "
            ...
            logger.info("PIN set from Tobi_PIN environment variable.")
            return custom_pin

    regen = os.getenv("Tobi_REGEN_PIN", "").lower() in ("true", "1", "yes")
