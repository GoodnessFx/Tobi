...
from Tobi.memory.conversation_store import (
    ConversationTurn,
...
from Tobi.agent.executor import AgentExecutor
from Tobi.agent.planner import TaskPlanner
from Tobi.agent.task_tracker import SubtaskStatus
from Tobi.agent.tools_schema import (
...
from Tobi.agent.coordinator import AgentCoordinator, AgentType
from Tobi.core.proactive import ProactiveEngine
from Tobi.core.hardening import sanitize_user_input
from Tobi.core.perf import perf_tracker, estimate_tokens, estimate_request_cost
from Tobi.core.dispatch_registry import DispatchRegistry, SuccessTracker
from Tobi.core.monitor import ConversationMonitor
from Tobi.agent.suggestions import suggest_followup, suggest_task_followup
from Tobi.agent.evolution_pipeline import EvolutionPipeline
from Tobi.tools.work_session import WorkSession

MAX_CONVERSATION_TURNS = 100

logger = logging.getLogger("Tobi.brain")
...
_CHAT_ONLY_PATTERNS = [
    ...
    # Conversational questions about Tobi or general small talk
...
# Patterns that indicate the user wants to shut down Tobi (intercepted to prevent system shutdown).
_Tobi_SHUTDOWN_PATTERNS = [
    # "shut down Tobi/javies/javis" (forgiving Whisper misspellings)
    r"\b(?:shut\s*down|shutdown|power\s*off|turn\s*off|mute|stop\s*listening|go\s*to\s*sleep)\s*(?:Tobi|javies?|javis|yourself|the\s*system|the\s*assistant)\b",
    r"\b(?:Tobi|javies?|javis|system)\s*(?:shut\s*down|shutdown|power\s*off|turn\s*off|mute|stop\s*listening)\b",
    r"\b(?:exit|quit|stop|terminate|kill|mute)\s*(?:Tobi|javies?|javis|yourself|the\s*system|the\s*assistant)\b",
    r"\b(?:go\s*(?:to\s*)?(?:sleep|offline)|power\s*down|mute|silence)\b",
    r"^(?:shut\s*down|shutdown|power\s*off|turn\s*off|mute|silence|stop)[\s!.,]*$",
    r"\b(?:shut\s*down|quit|exit|mute)\s*now\b",
]


def _is_Tobi_shutdown(text: str) -> bool:
    """Check if the user wants to shut down Tobi (not the computer)."""
    text_clean = text.strip().lower()
    for pattern in _Tobi_SHUTDOWN_PATTERNS:
...
def _select_tier(text: str) -> str:
    ...
    if len(text_lower) < 60:
        ...
        if not has_lookup:
            from Tobi.agent.planner import _count_action_verbs
...
class TobiBrain:
    ...
    async def initialize(self) -> bool:
        ...
        logger.info(
            "Tobi brain initialized (backend: %s, model: %s, restored %d turns).",
    ...
    async def process(self, user_input: str) -> str:
        ...
        if _is_Tobi_shutdown(user_input):
            logger.info("Tobi shutdown requested by user.")
            self._shutdown_requested = True
            return "Shutting down Tobi. All systems offline. Goodbye, sir."
        ...
        if hasattr(self, '_last_plan') and self._last_plan:
            ...
            try:
                suggestion = suggest_followup(
                    ...
                    working_dir=str(settings.Tobi_HOME),
                )
                if suggestion:
        ...
        try:
            return await self.llm.chat(
                ...
                system_prompt_override=(
                    "You are Tobi, a personal AI assistant. Summarize the results "
    ...
    async def shutdown(self):
        ...
        logger.info("Tobi brain shut down.")
