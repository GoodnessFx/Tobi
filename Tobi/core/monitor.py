"""
Conversation Quality Monitor - Real-time analysis of Tobi responses.

Analyzes Tobi responses for quality issues including response length,
character consistency (Tobi persona), formatting problems, and user
...
Quality checks focus on:
    ...
    - Tobi character consistency (specific phrases, tone)
...
logger = logging.getLogger("Tobi.monitor")

# Response quality thresholds
MAX_VOICE_SENTENCES = 4
MAX_VOICE_WORDS = 100

# Patterns that break Tobi character
CORPORATE_PHRASES = {
    "how can i help": "Tobi does not ask 'how can I help'; he just acts",
    "is there anything else": "Tobi does not ask 'is there anything else'",
    "i'd be happy to": "Too corporate; Tobi says 'Will do, sir' or just does it",
    "absolutely": "Tobi does not use filler enthusiasm like 'Absolutely'",
    "great question": "Tobi never says 'great question'",
    "as an ai": "Tobi never breaks character with 'as an AI'",
    "i cannot": "Tobi says 'I'm afraid that's beyond my current capabilities, sir'",
}
...
# Tobi should use "sir" frequently
...
class ConversationMonitor:
    """Analyzes Tobi response quality in real-time."""
    ...
    def analyze_response(self, user_text: str, Tobi_response: str) -> list[str]:
        """
        Analyze a Tobi response for quality issues.
        ...
        Args:
            ...
            Tobi_response: Tobi's response to analyze

        Returns:
            List of issue messages (empty if response is good)
        """
        self.total_analyzed += 1
        self.response_history.append(Tobi_response)
        self.user_history.append(user_text)

        found_issues = []

        # Check response length for voice compatibility
        voice_issues = self._check_voice_suitability(Tobi_response)
        found_issues.extend(voice_issues)

        # Check for corporate/character-breaking phrases
        character_issues = self._check_character_consistency(Tobi_response)
        found_issues.extend(character_issues)

        # Check for markdown/formatting issues
        formatting_issues = self._check_formatting(Tobi_response)
        ...
        memory_issues = self._check_memory_failure(user_text, Tobi_response)
        found_issues.extend(memory_issues)

        # Check for character breaches
        breach_issues = self._check_character_breach(Tobi_response)
    ...
    def _check_character_consistency(self, response: str) -> list[str]:
        """
        Check for phrases that break Tobi character.

        Args:
            response: Response text to check

        Returns:
            List of issue messages for corporate/non-Tobi phrases
        """
        issues = []
    ...
    def _check_sir_usage(self) -> list[str]:
        ...
        Tobi should use "sir" at least once in the last 5 responses.
        ...
        if recent_responses:
            ...
            if not sir_found:
                issues.append(
                    f"Tobi has not used 'sir' in the last {SIR_CHECK_WINDOW} responses; "
    ...
    def _check_memory_failure(self, user_text: str, Tobi_response: str) -> list[str]:
        """
        Check for memory failure patterns.

        User mentions earlier/before/remember + Tobi says "I don't recall".

        Args:
            user_text: User message
            Tobi_response: Tobi response
        ...
        issues = []
        user_lower = user_text.lower()
        response_lower = Tobi_response.lower()
        ...
        if has_memory_trigger and has_failure_response:
            issues.append(
                "Memory failure detected: user referenced past context but Tobi "
    ...
    def _check_character_breach(self, response: str) -> list[str]:
        ...
        if "samantha" in response_lower:
            issues.append(
                "Character breach: 'Samantha' mentioned; Tobi should not reference "
                "other AI systems"
            )
