// Background service worker: WebSocket connection to Tobi server
...
async function loadConfig() {
  try {
    const stored = await chrome.storage.local.get(["TobiConfig"]);
    if (stored.TobiConfig) {
      config = { ...DEFAULT_CONFIG, ...stored.TobiConfig };
    }
  } catch (e) {
    console.warn("[Tobi] Failed to load config from storage:", e);
  }
}
...
function connect() {
  ...
  console.log(`[Tobi] Connecting to ${config.serverUrl}...`);

  try {
    ws = new WebSocket(config.serverUrl);
  } catch (e) {
    console.error("[Tobi] WebSocket creation failed:", e);
    scheduleReconnect();
    return;
  }

  ws.onopen = () => {
    console.log("[Tobi] Connected to Tobi server.");
    isConnected = true;
    reconnectAttempts = 0;

    // Persist connected state so the alarm handler knows to maintain the connection
    chrome.storage.local.set({ TobiConnected: true });
  ...
  ws.onmessage = async (event) => {
    try {
      ...
      console.error("[Tobi] Failed to parse server message:", e, event.data);
    }
  };

  ws.onclose = (event) => {
    console.log(`[Tobi] Disconnected (code: ${event.code}, reason: ${event.reason})`);
    isConnected = false;
    ws = null;
    stopPingInterval();
    chrome.storage.local.set({ TobiConnected: false });
    updateBadge("off");
    scheduleReconnect();
  };

  ws.onerror = (error) => {
    console.error("[Tobi] WebSocket error:", error);
  };
}
...
function disconnect() {
  ...
  isConnected = false;
  chrome.storage.local.set({ TobiConnected: false, TobiUserDisconnected: true });
  updateBadge("off");
}
...
function scheduleReconnect() {
  ...
  if (reconnectAttempts >= config.maxReconnectAttempts) {
    console.warn("[Tobi] Max reconnect attempts reached. Relying on alarm for future retries.");
    updateBadge("off");
  ...
  console.log(`[Tobi] Reconnecting in ${Math.round(delay / 1000)}s (attempt ${reconnectAttempts})...`);
...
function ensureAlarm() {
  chrome.alarms.get(config.alarmName, (alarm) => {
    if (!alarm) {
      ...
      console.log("[Tobi] Keepalive alarm created.");
    }
  });
}
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  const stored = await chrome.storage.local.get(["TobiUserDisconnected"]);
  if (stored.TobiUserDisconnected) {
    return;
  }
  ...
  console.log("[Tobi] Alarm fired. Checking if Tobi server is reachable...");
  ...
  try {
    ...
    if (response.ok) {
      console.log("[Tobi] Server is reachable. Establishing WebSocket connection...");
      reconnectAttempts = 0; // Reset since the server is confirmed alive
      connect();
    } else {
      console.log(`[Tobi] Server responded with ${response.status}. Will retry on next alarm.`);
    }
  } catch (e) {
    // Server not reachable yet. Silently wait for next alarm cycle.
    console.log("[Tobi] Server not reachable. Will retry on next alarm.");
  }
});
...
function sendToServer(msg) {
  ...
  console.warn("[Tobi] Cannot send; not connected.");
  return false;
}
...
async function handleServerMessage(msg) {
  ...
  if (msg.type !== "command") {
    console.log("[Tobi] Non-command message:", msg.type);
    return;
  }
  ...
  try {
    ...
    console.error(`[Tobi] Command '${action}' failed:`, e);
    sendToServer({
      type: "result",
      id,
      success: false,
...
async function handleExecuteJs(code, tabId) {
  ...
  // Restrict JS execution to localhost and Tobi-related URLs only
  const tab = await chrome.tabs.get(id);
  ...
  if (!isAllowed) {
    return { success: false, error: "JS execution restricted to localhost and Tobi tunnel URLs." };
  }
...
async function routeToContentScript(msg) {
  ...
  try {
    ...
    console.debug("[Tobi] Content script injection note:", e.message);
  }
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "connect") {
    ...
    chrome.storage.local.set({ TobiUserDisconnected: false });
    reconnectAttempts = 0;
    connect();
  ...
  if (msg.type === "updateConfig") {
    ...
    chrome.storage.local.set({ TobiConfig: config });
    sendResponse({ ok: true });
    return true;
  }
...
chrome.runtime.onInstalled.addListener(() => {
  console.log("[Tobi] Extension installed/updated. Setting up keepalive alarm.");
  chrome.storage.local.set({ TobiUserDisconnected: false });
  ensureAlarm();
});

chrome.runtime.onStartup.addListener(() => {
  console.log("[Tobi] Chrome started. Setting up keepalive alarm.");
  chrome.storage.local.set({ TobiUserDisconnected: false });
  ensureAlarm();
});
...
loadConfig().then(() => {
  console.log("[Tobi] Background service worker started. Connecting...");
  ensureAlarm();
  connect();
});
