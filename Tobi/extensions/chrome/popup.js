...
versionEl.textContent = `Tobi Browser Bridge v${manifest.version}`;

function updateUI(status) {
  if (status.connected) {
    statusDot.className = "status-dot connected";
    statusText.textContent = "Connected to Tobi server";
...
btnSendPage.addEventListener("click", async () => {
  ...
  setTimeout(() => {
    btnSendPage.textContent = "Send this page to Tobi";
  }, 2000);
});
