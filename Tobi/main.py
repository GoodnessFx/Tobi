...
async def run_voice_mode():
    """Run Tobi in voice mode."""
    from Tobi.core.brain import TobiBrain
    from Tobi.voice.listener import VoiceListener
    from Tobi.voice.speaker import VoiceSpeaker

    brain = TobiBrain()
    listener = VoiceListener()
    speaker = VoiceSpeaker()

    logger.info("Initializing Tobi components...")
    ...
    await speaker.speak("Tobi online. All systems operational. How can I help you?")
...
async def run_text_mode():
    ...
    print("\nTobi is ready. Type your message (or 'quit' to exit).\n")

    if speaker_ok:
        await speaker.speak("Tobi online. How can I help you?")
    ...
    while True:
        ...
        if user_input.lower() in ("quit", "exit", "bye", "goodbye"):
            print("\nTobi: Goodbye. Shutting down systems.")
            if speaker_ok:
...
async def run_server_mode():
    """Run Tobi as API server."""
    import uvicorn
    from Tobi.core.server import app

    ssl_certfile = os.environ.get("Tobi_TLS_CERT")
    ssl_keyfile = os.environ.get("Tobi_TLS_KEY")
    uvicorn_kwargs = dict(
        app=app,
...
async def run_full():
    ...
    import uvicorn
    from Tobi.core.server import app, brain, broadcast_voice_interaction, broadcast_voice_state, broadcast_voice_chunk, broadcast_overlay_state, set_voice_components
    from Tobi.voice.listener import VoiceListener
    from Tobi.voice.speaker import VoiceSpeaker
    ...
    async def run_voice_with_shared_brain():
        ...
        def _needs_async_execution(text: str) -> bool:
            ...
            """
            from Tobi.core.brain import _select_tier, _is_chat_only
...
def _display_auth_info():
    ...
    from Tobi.core.server import get_startup_pin
    pin = get_startup_pin()
    if pin:
        print("  ==========================================")
        ...
        print("  (Set Tobi_REGEN_PIN=true to generate a new PIN)")
        print()
...
def main():
    ...
    if mode == "voice":
        ...
        print("  Other modes:")
        print("    python -m Tobi.main voice    (voice interaction)")
        print("    python -m Tobi.main server   (API server only)")
        print("    python -m Tobi.main full     (voice + API server)")
        print()
