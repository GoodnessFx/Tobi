...
logger = logging.getLogger("Tobi.tools.browser_agent")
...
DOWNLOAD_DIR = Path.home() / "Downloads" / "Tobi"

Tobi_HOME = Path(__file__).parent.parent.parent
BROWSER_PROFILE_DIR = Tobi_HOME / "data" / "browser-profile"
...
class BrowserAgent:
    ...
    async def initialize(self, sync_chrome: bool = True) -> bool:
        ...
        try:
            ...
            if sync_chrome and self._context:
                try:
                    from Tobi.tools.chrome_sync import sync_chrome_cookies
    ...
    async def navigate(self, url: str) -> str:
        ...
        try:
            if self._context:
                try:
                    from Tobi.tools.chrome_sync import sync_chrome_for_url
...
async def sync_browser_sessions(domains: str = "") -> str:
    """Sync login sessions from Chrome into Tobi's browser."""
    agent = _get_browser_agent()
    ok = await agent._ensure_ready()
    if not ok:
