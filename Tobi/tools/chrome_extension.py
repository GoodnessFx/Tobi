"""WebSocket bridge to Tobi Chrome Extension for DOM-based browser automation."""
...
logger = logging.getLogger("Tobi.tools.chrome_extension")
...
async def _send_command(action: str, **params) -> dict:
    ...
    if not is_extension_connected():
        raise ConnectionError(
            ...
            "Install and enable the Tobi Browser Bridge extension in Chrome."
        )
...
async def chrome_navigate(url: str, tab_id: int = None, new_tab: bool = False) -> str:
    """Navigate a Chrome tab to a URL; avoids navigating Tobi UI tabs."""
    try:
            ...
            try:
                ...
                if tabs_result.get("success"):
                    ...
                    if active_tab:
                        ...
                        is_Tobi_tab = any(p in active_url for p in Tobi_patterns)
                        if is_Tobi_tab:
                            # Find an existing non-Tobi, non-extension tab to reuse
                            reuse_tab = None
                            for t in tabs:
                                t_url = (t.get("url") or "").lower()
                                skip = (
                                    any(p in t_url for p in Tobi_patterns)
                            ...
                            if reuse_tab:
                                tab_id = reuse_tab["id"]
                                logger.info(
                                    "Active tab is Tobi UI; reusing tab %s instead.",
                                    tab_id,
                                )
                            else:
                                new_tab = True
                                logger.info(
                                    "Active tab is Tobi UI; opening new tab for navigation."
                                )
