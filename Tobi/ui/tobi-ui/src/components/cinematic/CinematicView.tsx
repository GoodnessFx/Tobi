...
import { unlockAudio } from "@/hooks/useTobiWebSocket";
...
const stateColors: Record<OrbState, string> = {
  idle: "text-Tobi-cyan/40",
  listening: "text-Tobi-cyan/70",
  thinking: "text-Tobi-gold/60",
  speaking: "text-Tobi-gold/70",
  error: "text-Tobi-error/60",
};
  ...
  return (
    <div className="flex-1 relative overflow-hidden bg-black">
      ...
      {!showBoot && (
        ...
        >
          <div className={`w-1.5 h-1.5 rounded-full ${
            orbState === 'idle' ? 'bg-Tobi-cyan/30' :
            orbState === 'listening' ? 'bg-Tobi-cyan/60 animate-pulse' :
            orbState === 'thinking' ? 'bg-Tobi-gold/50 animate-pulse' :
            orbState === 'speaking' ? 'bg-Tobi-gold/60' :
            'bg-Tobi-error/50'
          }`} />
      ...
      {!showBoot && (
        <button
          ...
          className={`
            ...
            ${showTranscript
              ? "bg-Tobi-cyan/6 border-Tobi-cyan/12 text-Tobi-cyan/50"
              : "bg-white/[0.02] border-white/[0.04] text-Tobi-text-dim/20 hover:text-Tobi-text-dim/40"
            }
          `}
          style={{ zIndex: 20 }}
      ...
      {!showBoot && showTranscript && latestAssistant && (
        ...
        >
          <div className="Tobi-glass-subtle text-center px-5 py-3 rounded-2xl">
            <div className="text-xs leading-relaxed text-Tobi-text/50">
      ...
      {!showBoot && orbState === "speaking" && !isRecording && (
        ...
        >
          <div className="flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-Tobi-gold/50 animate-pulse" />
            <span className="text-3xs font-mono text-Tobi-gold/40 uppercase tracking-[0.15em]">
              Speaking
            </span>
          </div>
        </div>
      ...
      {!showBoot && micSupported && (
        ...
        >
          {micError && (
            <div className="Tobi-glass-subtle px-3 py-1.5 rounded-xl">
          ...
          {(isRecording || isTranscribing) && (
            <span className="text-3xs font-mono uppercase tracking-[0.15em] text-Tobi-cyan/50 animate-fade-in">
          ...
          <button
            ...
            className={`
              ...
              ${isRecording
                ...
                : isTranscribing
                  ? "bg-Tobi-cyan/8 border-Tobi-cyan/20 text-Tobi-cyan/50"
                  : "bg-white/[0.03] border-white/[0.08] text-Tobi-text-dim/40 hover:text-Tobi-cyan hover:border-Tobi-cyan/30 hover:bg-Tobi-cyan/5 hover:shadow-[0_0_20px_rgba(0,190,255,0.12)]"
              }
