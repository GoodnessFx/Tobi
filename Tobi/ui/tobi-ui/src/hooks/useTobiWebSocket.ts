...
function getAudioElement(): HTMLAudioElement {
  if (!_audioEl) {
    ...
    console.log("[Tobi Audio] Hidden <audio> element created");
  }
  return _audioEl;
}
...
export function ensureAudioContext(): AudioContext {
  if (!_audioCtx) {
    ...
    console.log("[Tobi Audio] AudioContext created, state:", _audioCtx.state);
  }
  return _audioCtx;
}
...
export async function unlockAudio(): Promise<void> {
  ...
  if (ctx.state === "suspended") {
    await ctx.resume();
    console.log("[Tobi Audio] AudioContext resumed from user gesture");
  }
  try {
  ...
  try {
    ...
    await el.play().catch(() => {});
    console.log("[Tobi Audio] <audio> element primed from user gesture");
  } catch (_) { /* ignore */ }
}
...
async function playBase64Audio(base64Wav: string, mimeType: string = "audio/wav"): Promise<void> {
  ...
  try {
    ...
    el.onplaying = () => {
      if (_onAudioPlaybackStarted) {
        ...
        console.log("[Tobi Audio] <audio> onplaying fired, real duration=%.2fs", realDur);
    ...
    console.log("[Tobi Audio] Playing via <audio> element (%d KB)", audioSizeKB);
    ...
    console.warn("[Tobi Audio] <audio> element playback failed, trying Web Audio API:", e);
  }
  ...
  try {
    ...
    if (_onAudioPlaybackStarted) {
      console.log("[Tobi Audio] Web Audio API started, real duration=%.2fs, syncing envelope", audioBuffer.duration);
      _onAudioPlaybackStarted(audioBuffer.duration);
      _onAudioPlaybackStarted = null;
    }

    console.log("[Tobi Audio] Playing via Web Audio API (%d KB, %.1fs)", audioSizeKB, audioBuffer.duration);
  } catch (e) {
    console.warn("[Tobi Audio] Both playback strategies failed:", e);
...
interface UseTobiWebSocketReturn {
...
export function useTobiWebSocket(authToken?: string | null): UseTobiWebSocketReturn {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        console.log("[Tobi WS] Sent client registration:", deviceInfo);
      };
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.error) {
            console.error("[Tobi WS] Server error:", data.error);
          ...
          if (data.client_registered) {
            console.log("[Tobi WS] Client registered. Connected devices:", data.connected_devices?.length);
          ...
          if (data.audio_preference_updated) {
            console.log("[Tobi WS] Audio preference updated: wants_audio=%s", data.wants_audio);
            return;
          }

          // Audio interruption signal from server (or another client)
          if (data.voice_stop) {
            console.log("[Tobi WS] Audio stop signal received, interrupting playback");
          ...
          if (data.voice_audio_chunk) {
            ...
            if (chunk.index === 0) {
              ...
              console.log("[Tobi Audio] First chunk received, starting streamed playback");
            }
            ...
            const playNextChunk = () => {
              ...
              el.play().catch((e) => {
                console.warn("[Tobi Audio] Chunk playback failed:", e);
                isPlayingChunkRef.current = false;
              ...
              console.log(
                "[Tobi Audio] Playing chunk %d (%.1fs, %d KB, %s)",
          ...
          if (data.voice_speaking !== undefined) {
            ...
            if (data.voice_speaking && data.voice_audio) {
              ...
              console.log(
                "[Tobi WS] Received voice_audio (%d KB). AudioContext state: %s",
                audioSizeKB,
              ...
              if (data.amplitude_envelope && data.audio_duration) {
                ...
                const startEnvelopeTick = (realDuration: number) => {
                  ...
                  if (realDuration > 0) {
                    console.log(
                      "[Tobi Audio] Overriding server duration (%.2fs) with real audio duration (%.2fs)",
                  ...
                  envelopeStartRef.current = performance.now();
                  console.log("[Tobi Audio] Envelope animation started (synced, dur=%.2fs)", envelopeDurationRef.current);
              ...
              console.log("[Tobi WS] voice_speaking=true but NO voice_audio or envelope");
            }
          ...
          console.error("[Tobi WS] Parse error:", e);
        }
      };
      ...
      ws.onclose = () => {
        ...
        wsRef.current = null;
        console.log("[Tobi WS] Disconnected");

        // Auto-reconnect
        if (reconnectAttempts.current < MAX_RECONNECT_ATTEMPTS) {
          reconnectAttempts.current += 1;
          const delay = RECONNECT_DELAY_MS * Math.min(reconnectAttempts.current, 5);
          console.log(`[Tobi WS] Reconnecting in ${delay}ms (attempt ${reconnectAttempts.current})`);
          reconnectTimeout.current = setTimeout(connect, delay);
        }
      };

      ws.onerror = (err) => {
        console.error("[Tobi WS] Error:", err);
        setStatus("error");
      };
    } catch (e) {
      console.error("[Tobi WS] Connection failed:", e);
      setStatus("error");
    }
  }, [authToken]);
  ...
  const sendMessage = useCallback((text: string) => {
    if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
      console.error("[Tobi WS] Cannot send: not connected");
      return;
    }
